        ...
        with open(filepath, "r", newline="", encoding="utf-8") as infile:
            ...
            try:
                ...
                if not any(field.strip() for field in header):
                    raise ValueError(
                        "Header row appears to be empty or contain only whitespace."
                    )
                return header
...
if not csv_files:
    print(
        f"No .csv files found in '{os.path.abspath(TARGET_DIRECTORY)}' (excluding '{OUTPUT_FILENAME}')."
    )
    sys.exit(0)
...
for filename in csv_files:
    ...
    try:
        ...
        if reference_header is None:
            ...
            print(
                f" OK (Reference header with {len(reference_header)} columns established)"
            )
            ...
            print(
                f"File '{filename}' header does not match the reference header from '{first_valid_file}'."
            )
            ...
            try:
                with open(file_path, "r", newline="", encoding="utf-8") as infile:
                    ...
                    try:
                        ...
                        print(
                            f" Error: CSV formatting error while reading data near line {csv_reader.line_num}: {csv_e}. Skipping rest of this file."
                        )
                ...
                print(
                    f" Error: Could not read file during data append phase: {io_e}. Skipping."
                )
                files_skipped_in_data_phase += 1
            except Exception as e:
                # Unexpected error during data append for this file
                print(
                    f" Error: An unexpected error occurred appending data: {e}. Skipping."
                )
                files_skipped_in_data_phase += 1
